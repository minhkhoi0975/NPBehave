...
public class NPBehaveExampleSwarmEnemyAI : MonoBehaviour
{
                    ...
                    new BlackboardCondition("engaged", Operator.IS_EQUAL, true, Stops.IMMEDIATE_RESTART,
                        ...
                        new Sequence(
                            ...
                            new Action((bool _shouldCancel) =>
                            {
                                if (!_shouldCancel)
                                {
                                    MoveTowards(ownBlackboard.GetBlackboardValue<Vector3>("playerLocalPos"));
    ...
    private void UpdateBlackboards()
    {
        ...
        if (ownBlackboard.GetBlackboardValue<bool>("playerInRange") && !ownBlackboard.GetBlackboardValue<bool>("engaged") && sharedBlackboard.GetBlackboardValue<int>("numEngagedActors") < 2)
        {
            // increment the shared 'numEngagedActors'
            sharedBlackboard["numEngagedActors"]= sharedBlackboard.GetBlackboardValue<int>("numEngagedActors") + 1;
        ...
        if (!ownBlackboard.GetBlackboardValue<bool>("playerInRange") && ownBlackboard.GetBlackboardValue<bool>("engaged"))
        {
